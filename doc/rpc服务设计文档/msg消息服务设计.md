...
1. client A -> server  上行可靠性
   1. A向服务器发送消息（带timeout），消息（附带**本地Id**），每次连接从1开始
   2. * 服务器收到消息并成功写入数据库，向A发送ack（消息不丢）
      * 服务器未收到消息，A超时显示**未发送**，由A主动发起重传
   3. * 若1，2丢失，A 都要重传
      * 对于2，根据本地id，若小于服务器序列号，不处理（去重）
2. server -> client B  下行可靠性
   1. * server 在一定时间内重试，向client 发送，超时重新加入历史消息
      * 在线，直接向B发送
      * 不在线，存储到历史消息队列，B上线时主动从队列拉取
   2. B收到消息向服务器发送ack（附带**msgId**），从队列中删去此消息
   3. 服务器收到ack，向B发送Ack
   4. * 若1丢失，消息在离线消息内
      * 若2，3 丢失，B要进行重传，若重传了重复的msgId，从离线消息里会找不到，无影响。
3. 离线拉取
   1. client B 向服务器发送拉取请求
   2. server 发送批量离线
   3. client 发送 ack
   4. server发送ack
